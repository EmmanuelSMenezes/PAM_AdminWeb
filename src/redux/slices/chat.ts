...
// utils
import { api, apiCommunication } from '../../service/api';
// @types
...
export async function createChat(chat: {order_id: null; members: string[]}) {
  const response = await apiCommunication.post('/chat/create', chat);
  return response.data.data;
}
...
export function markConversationAsRead(conversationId: string) {
  return async (dispatch: Dispatch) => {
    dispatch(slice.actions.startLoading());
    try {
      await api.get('/api/chat/conversation/mark-as-seen', {
